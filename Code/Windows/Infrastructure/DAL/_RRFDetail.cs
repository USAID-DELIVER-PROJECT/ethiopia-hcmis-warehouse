...
namespace DAL
{
	public abstract class _RRFDetail : SqlClientEntity
	{
		...
		public bool LoadUpdatesAfter( long LastVersion ){
			this.FlushData();
			string q = string.Format(@"SELECT T.* FROM RRFDetail AS T Where T.ID in (SELECT CT.ID FROM CHANGETABLE(CHANGES RRFDetail, {0}) AS CT)", LastVersion);
		...
		public bool LoadUpdatesAfterByTime( DateTime LastUpdateTime ){
			this.FlushData();
			string q = string.Format(@"SELECT T.* FROM RRFDetail AS T Where T.UpdateTime > {0} AS CT)", LastUpdateTime);
			return base.LoadFromRawSql(q);
		}
		
		public  bool LoadDeletedIDs( long LastVersion) {
			this.FlushData();
			string q = string.Format(@"SELECT ID FROM CHANGETABLE (CHANGES RRFDetail,{0}) as CT where SYS_CHANGE_OPERATION = 'D' ORDER BY SYS_CHANGE_VERSION" , LastVersion);
			return base.LoadFromRawSql(q);
		}
		...
		{  
			...
			static public string ToPropertyName(string columnName)
			{
				if(ht == null)
				{
					...
					ht[ActivityID] = _RRFDetail.PropertyNames.ActivityID;
		...
		{  
			...
			static public string ToColumnName(string propertyName)
			{
				if(ht == null)
				{
					...
					ht[ActivityID] = _RRFDetail.ColumnNames.ActivityID;
